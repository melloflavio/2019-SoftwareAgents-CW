...
As stated in section \ref{sec:transition}, the program makes use of this base transition graph to generate the full transition matrix. Given the agent's independence in terms of movement and policy, it was decided during implementation to have a separate transition matrix for each agent. Similarly, each agent has a separate R matrix associated with it. Even if, for the purposes of this study the agents are deemed cooperative and share the same final goal, their transitions into the final state are different. The R matrixes are generated by taking the agent's transition matrix, setting all transitions to have return of 0, then all transitions away from the desired terminal state to -100 and, finallly, all transitions into the terminal state to 100. Figure \ref{fig:r_matrix} shows a section of the R matrix generated for agent #1 in a experiment with 2 agents.

	\begin{figure}[h]
		\centering
		\includegraphics[height=4cm]{./Images/5_R_Matrix.jpg}
		\caption{Section of R Matrix generated for agent 1}
		\label{fig:r_matrix}
	\end{figure}
